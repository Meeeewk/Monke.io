...
public class Bot extends MovingEntity {
	...
	private MovingEntity fakeTarget;
	...
	public Bot(double x, double y, int width, int height, int moveSpeedMod, String imgPath, int drawWidth,
		...
		this.lazyLength = 600 / this.getDrawWidth() * 220;
	}
...
//		this.setDrawWidth(rnd);
	...
	public void move() {
		...
		// Roaming mode if doesn't have a target, or if target is too far away / too
		// high away
		if (target == null || vectLen > (lazyLength * this.getDrawHeight() / 200) + this.getDrawHeight() / 4
				|| Math.abs(z - this.getZ()) > 1) {
...
//	    g2d.fillOval((int) (xRoam * 3.5 + this.getX() - playerX + this.getWidth() / 2), (int) (yRoam * 3.5 + this.getY() - playerY + this.getHeight() / 2),30,30);
		...
		g2d.fillOval((int) (this.getX() - playerX - (this.getDrawWidth() * 0.72) / 2.0 + this.getWidth() / 2),
...
//	    g2d.drawString("chunk: " + Arrays.toString(getChunk()), x, y-30);
		...
		g2d.fillRect(x + ((int) (this.getHealth() * this.getDrawWidth() / 100)), y,
				(int) ((100 - this.getHealth()) * this.getDrawWidth() / 100), 10);
		this.setHitCooldown(this.getHitCooldown() <= 0 ? 0 : this.getHitCooldown() - 0.5);
		this.setMoveSpeedMod(this.getHitCooldown() == 0 ? 140+((int)MovingEntity.skinSpeed[this.getSkin()]*3) : targeting?100+((int)MovingEntity.skinSpeed[this.getSkin()]*3): 40);
		this.setMaxSpeed(
				targeting ? MovingEntity.skinSpeed[this.getSkin()] : MovingEntity.skinSpeed[this.getSkin()] / 1.4);
	}
